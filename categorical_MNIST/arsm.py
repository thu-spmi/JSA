...
# Apache 2.0.
# This script contrains ARSM for categorical MNIST experiment.
...
def get_nll(sess,data,total_batch):
    ...
    for j in range(total_batch):
        xs, _ = data.next_batch(batch_size)
        cost_eval.append(sess.run(nll_loss, {x: np.tile(xs,[n_for_nll,1])}))
    return np.mean(cost_eval)
...
def get_gradient_logvars(sess, xs, samples=1000):
    ...
    for i in range(samples):
        dirich, logits = sess.run([pai, z_concate], {x: xs})
        FF = compt_F(sess, dirich, logits, xs)
        flat_qgrad_temp,flat_pgrad_temp = sess.run([flat_qgrad,flat_pgrad], {x: xs, F: FF, PAI: dirich})
