...
def get_gradient_logvars(sess, xs,ys, samples=1000):
    ...
    for i in range(samples):
        ...
        flat_qgrad_temp, flat_pgrad_temp = sess.run([flat_qgrad,flat_pgrad], {x: xs, ntimes: n, b_sample_in: choose_samples})
...
train_data._labels=np.arange(0,len(train_data._labels)) #labels records the index of each training datapoint, for recording the cache samples
...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        _,cost = sess.run([train_op,gen_loss],{x:train_xs,ntimes:n, b_sample_in:choose_samples})
